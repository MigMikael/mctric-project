<?php
return [
    'welcome'   => 'ยินดีต้อนรับเข้าสู่แอพของเรา',
    'app'   => [
        'homeMenu' =>  'หน้าหลัก',
        'aboutMenu' =>  'เกี่ยวกับเรา',
        'businessMenu' => 'ธุรกิจของเรา',
        'careerMenu' => 'ตำแหน่งงาน',
        'clientMenu' => 'ลูกค้าของเรา',
        'awardMenu' => 'รางวัลและการรับรองของเรา',
        'contactMenu' => 'ติดต่อเรา',
        'chequeTrackMenu' => 'ติดตามเช็ค',
        'dashboardMenu' => 'แดชบอร์ด',
        'logoutMenu' => 'ล็อกเอ้าท์',
        'loginMenu' => 'ล็อกอิน',

        'createButton' => 'สร้าง',
        'editButton' => 'แก้ไข',
        'deleteButton' => 'ลบ',
        'searchButton' => 'ค้นหา',
    ],
    'home' => [
        'webTitle' => 'บริษัท แมคทริค จำกัด (มหาชน)',
        'companyTitle' => 'แมคทริค',
        'companySubtitle' => 'บริษัท แมคทริค จำกัด (มหาชน)',
        'watchStoryButton' => 'ดูเรื่องราวของเรา',

        'aboutHeading' => 'เกี่ยวกับเรา',
        'aboutParagraphOne' => 'McTRIC เราคือผู้รับเหมาที่ก้าวหน้าไปพร้อมกับการเปลี่ยนแปลงของโลก ที่มีระบบดิจิทัลเข้ามาเกี่ยวข้องในหลายๆด้าน ตลอดจนการพัฒนางานด้านวิศวกรรมให้บริษัทสามารถสนับสนุนธุรกิจของลูกค้า และพันธมิตร ก้าวไปสู่ความยั่งยืนและเป็นมิตรกับสิ่งแวดล้อม',
        'aboutParagraphTwo' => 'เราจึงได้พัฒนาธุรกิจไปในหลายๆด้านโดยเฉพาะด้าน Automation เพื่อสนับสนุนธุรกิจ E-commerce
        ที่เติบโตอย่างรวดเร็ว ตลอดจนการออกแบบและให้การปรึกษาแก่ธุรกิจพลังงานแสงอาทิตย์เป็นต้น',
        'aboutParagraphThree' => 'McTRIC เราไม่เคยหยุดนิ่ง พัฒนาตัวเองและเปลี่ยนแปลงตัวเองให้ทันกับโลกที่เปลี่ยนไปอย่างไม่หยุดยั้ง',

        'philosophyHeading' => 'วิสัยทัศน์ของเรา',
        'philosophyParagraphOne' => 'คุณภาพที่ดีเลิศ',

        'missionHeading' => 'พันธกิจของเรา',
        'missionOne' => 'การวางแผนงานที่ดี',
        'missionTwo' => 'วิธีการทำงานที่ถูกต้อง',
        'missionThree' => 'งานไม่ล่าช้า',

        'objectiveHeading' => 'วัตถุประสงค์',
        'objectiveOne' => 'ขยายงานรับเหมาติดตั้งงานระบบให้เติบโตอย่างต่อเนื่อง',
        'objectiveTwo' => 'ผลงานได้คุณภาพ ภายใต้งบประมาณและเสร็จตามกำหนดเวลาที่สัญญาไว้กับลูกค้า',
        'objectiveThree' => 'ควบคุมระบบการปฎิบัติงานให้ได้ตามมาตรฐานและเกิดการพัฒนาอย่างต่อเนื่อง',
        'objectiveFour' => 'บริหารจัดการระบบภายในบริษัท ให้มีประสิทธิภาพ',
        'objectiveFive' => 'พัฒนาความรู้ความสามารถของบุคลากรอย่างต่อเนื่อง',

        'policyHeading' => 'นโยบายคุณภาพ',
        'policyDescription' => 'พัฒนาอย่างต่อเนื่อง เพื่อคุณภาพที่มั่นคง และความประทับใจของลูกค้า',

        'businessHeading' => 'ธุรกิจของเรา',
        'civilMenu' => 'งานโยธาและก่อสร้าง',
        'mechanicalMenu' => 'บริการเครื่องกลไฟฟ้าและประปา (MEP)',
        'utilityMenu' => 'ท่อสาธารณูปโภค',
        'renewableMenu' => 'พลังงานทดแทน',
        'supplyMenu' => 'ระบบอัตโนมัติของซัพพลายเชน',
        'coProjectMenu' => 'โปรเจคร่วม',
        'jointVentureMenu' => 'โปรเจคร่วม',
        'businessButton' => 'ดูธุรกิจทั้งหมดของเรา',

        'careersHeading' => 'ร่วมงานกับเรา',
        'careersSubHeading' => 'ทำไมต้องร่วมงานกับเรา ?',
        'careersParagraphOne' => 'บริษัทฯ มุ่งมั่นที่จะเป็นผู้เชี่ยวชาญในงานด้านวิศวกรรม งานรับเหมาก่อสร้าง และการติดตั้งระบบในแขนงต่างๆเพื่อส่งมอบผลงานที่มีคุณภาพแก่ลูกค้า',
        'careersParagraphTwo' => 'ประกอบธุรกิจโดยคำนึกถึงผู้มีส่วนได้ส่วนเสีย และรับผิดชอบต่อสังคม ลงทุนและพัฒนาบุคคลากรของบริษัทฯอย่างต่อเนื่อง เรากำลังมองหาผู้ร่วมอุดมการณ์ที่จะมาร่วมสร้างสรรค์ผลงานที่มีคุณภาพกับเรา',
        'careersButton' => 'ดูตำแหน่งงานทั้งหมด',

        'clientHeading' => 'ลูกค้าของเรา',
        'clientButton' => 'ดูลูกค้าทั้งหมดของเรา',

        'awardHeading' => 'รางวัลและการรับรองของเรา',
        'awardDescription' => 'บริษัทของเราได้รับรางวัลและการรับรองจากองค์กรเพื่อรับประกันคุณภาพ มาตรฐานและบริการตลอดจนทุกกระบวรการของบริษัท เพื่อให้ลูกค้ามั่นใจในคุณภาพและบริการของเรา',
        'awardButton' => 'ดูรางวัลของเราทั้งหมด',

        'companyAnnouncementHeading' => 'ประกาศบริษัท',
        'companyAnnouncementDescription' => 'ติดตามข่าวสารและประกาศล่าสุดของบริษัท พบกับข้อมูลสำคัญ ความเคลื่อนไหวใหม่ๆ และพัฒนาการที่สำคัญที่จะกำหนดทิศทางในอนาคตของเรา',

        'contactHeading' => 'ติดต่อเรา',
        'contactAddress' => 'ที่อยู่',
        'contactAddressDesc' => '121/105 อาคารอาร์เอส ทาวเวอร์ ชั้น 39 ถนนรัชดาภิเษก แขวงดินแดง เขตดินแดง กรุงเทพมหานคร 10400',
        'contactEmail' => 'อีเมลหาเราได้ที่',
        'contactEmailDesc' => 'admin@mctric.com',
        'contactCall' => 'โทรหาเรา',
        'contactFax' => 'แฟกซ์',
        'contactPhone' => 'โทร',

        'chequeTracking' => 'ติดตามเช็ค',

        'copyright' => 'ลิขสิทธิ์',
    ],
    'business' => [
        'businessHeading' => 'ธุรกิจของเรา',
        'civilMenu' => 'งานโยธาและก่อสร้าง',
        'mechanicalMenu' => 'บริการเครื่องกลไฟฟ้าและประปา (MEP)',
        'utilityMenu' => 'ท่อสาธารณูปโภค',
        'renewableMenu' => 'พลังงานทดแทน',
        'supplyMenu' => 'ระบบอัตโนมัติของซัพพลายเชน',
        'jointVentureProject' => 'โปรเจคร่วม',

        'client' => 'ลูกค้า',
        'designer' => 'ผู้ออกแบบ',
        'consultant' => 'ที่ปรึกษา',
        'contractor' => 'ผู้รับเหมา',
        'mainContractor' => 'ผู้รับเหมาหลัก',
        'contractPeriod' => 'ระยะเวลาของสัญญา',
        'contractValue' => 'มูลค่าสัญญา',
        'scopeOfWork' => 'ขอบเขตงาน',
        'projectPhoto' => 'ภาพโครงการ',
        'description' => 'รายละเอียดโครงการ',
        'bath' => 'บาท',
        'project' => 'โครงการ',

        'businessReadMore' => 'อ่านเพิ่มเติม',

        'filterByYear' => 'เลือกตามปี'
    ],
    'dashboard' => [
        'dashboardHeading' => 'แอดมินแดชบอร์ด',

        'summaryTab' => 'สรุป',
        'summarySubHeading' => 'สรุปสถานะธุรกิจ',
        'businessTab' => 'ธุรกิจของเรา',
        'businessSubHeading' => 'จัดการธุรกิจ',
        'careerTab' => 'ตำแหน่งงาน',
        'careerSubHeading' => 'จัดการตำแหน่งงาน',
        'clientTab' => 'ลูกค้า',
        'clientSubHeading' => 'จัดการลูกค้า',
        'awardTab' => 'รางวัล',
        'awardSubHeading' => 'จัดการรางวัล',
        'userTab' => 'ผู้ใช้งาน',
        'userSubHeading' => 'จัดการผู้ใช้งาน',

        'cardButton' => 'รายละเอียด',
        'sortButton' => 'เรียงลำดับ',
        'hideButton' => 'ซ่อน',
        'showButton' => 'แสดง',
        'saveButton' => 'บันทึก'
    ],
    'career' => [
        'careerHeading' => 'ตำแหน่งงาน',
        'jobOpening' => 'ตำแหน่งงานที่เปิดรับ',
        'readMoreButton' => 'อ่านเพิ่มเติม',
        'description' => 'รายละเอียด',
        'qualification' => 'คุณสมบัติ',
        'applyBtn' => 'สมัคร'
    ],
    'client' => [
        'clientHeading' => 'ลูกค้าของเรา',
        'clientSubHeading' => 'ลูกค้าของเรา',
    ],
    'award' => [
        'awardHeading' => 'รางวัลและการรับรองของเรา',
        'awardSubHeading' => 'รางวัลและการรับรอง',
    ],
    'contact' => [
        'contactHeading' => 'ติดต่อเรา',
        'contactAddress' => 'ที่อยู่',
        'contactEmail' => 'อีเมลหาเราได้ที่',
        'contactCall' => 'โทรหาเรา',
        'contactPhone' => 'โทร',
        'contactFax' => 'แฟกซ์',
        'contactCompanyName' => 'บริษัท แมคทริค จำกัด (มหาชน)',
    ],
    'status' => [
        'workInProgress' => 'ระหว่างการดำเนินการ',
        'complete' => 'เสร็จสิ้น'
    ]
];
